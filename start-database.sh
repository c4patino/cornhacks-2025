...
DB_CONTAINER_NAME="cornhacks-2025-postgres"

if ! [ -x "$(command -v podman)" ]; then
  echo -e "Podman is not installed. Please install podman and try again.\nPodman install guide: https://podman.io/docs/installation."
  exit 1
fi
...
if [ ! -f .env ]; then
    echo ".env file was not found. Environment variables are required to initialize the postgres database."
    exit 1
fi
